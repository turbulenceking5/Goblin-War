...
A browser-based overworld RPG. No build step, no framework — every page is a single self-contained `.html` file with inline `<style>` and `<script>`, opened directly or served as static files. All game state lives in `localStorage`, shared across pages by convention (same key names, no formal schema). The one exception is Supabase (hosted Postgres + auth), talked to directly over its JS client loaded from a CDN `<script>` tag — no server code of ours, just a public anon key and Row Level Security — for two things: **every player page requires a logged-in account** (see [context/accounts.md](context/accounts.md)), and settings.html's Account section offers manual cloud save/load on top of that (see [context/save-system.md](context/save-system.md)).
...
| [settings.html](settings.html) | 3 save slots (read/write `localStorage` snapshots), a new-game reset, the Change Log, and cloud save/load + Log Out (see [context/accounts.md](context/accounts.md)). |
| [login.html](login.html) | Sign up / log in (email + password via Supabase Auth). Every other player page redirects here if there's no valid session; logging in sends the player back to wherever they came from (`?redirect=`) or index.html. See [context/accounts.md](context/accounts.md). |
...
| `assets/auth-gate-sync.js` | The synchronous, no-network first pass of the login gate — loaded right after supabase-config.js in every gated page's `<head>`, before any body content. See [context/accounts.md](context/accounts.md). |
| `assets/auth-client.js` | The authoritative (async) half of the login gate — creates the shared `sb` client and `authGateReady` promise every page's own script uses. Loaded near the bottom of every gated page, after the Supabase JS CDN script. See [context/accounts.md](context/accounts.md). |
| `supabase/schema.sql` | The `saves` table + Row Level Security policies backing cloud save. Not run automatically — paste it into the Supabase dashboard's SQL Editor once per project. Kept here so the schema is versioned. |
...
- **Shipping a change? Add a `CHANGELOG` entry — in three places.** The `CHANGELOG` array (id/date/changes) is duplicated identically in index.html (drives the "What's New" popup shown on map load once `goblinwar_lastSeenChangelog` falls behind the newest id) and settings.html (renders the full history under Change Log, and bumps that same key so opening Settings doesn't also trigger the popup). Add one new `{id, date, changes}` entry — id one higher than the last — to both arrays whenever a player-visible change ships, or the two pages will disagree about what's new. Also bump `version.json`'s `changelogId` to match — that's what index.html's "Update available" banner polls to know a newer build exists on the server; forgetting it means the banner never fires for that release.
