...
| [settings.html](settings.html) | 3 save slots (read/write `localStorage` snapshots), a new-game reset, and the Change Log. |
...
| `assets/enemies/` | Real portrait art for combat (`#combat-enemy-icon`), one PNG per enemy variant (e.g. `goblin-raider.png`) — transparent background, downscaled to a 600px longest edge. Only Goblin and Ork have real art so far; Bandit still falls back to a hand-drawn SVG. See [context/combat.md](context/combat.md)'s `ENEMY_VARIANTS`. |
