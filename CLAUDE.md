...
| `assets/icons/` | `icon-192.png` / `icon-512.png` (manifest) and `icon-180.png` (`apple-touch-icon`) — a simple gold fortress-on-dark glyph, generated once (see the git history for `assets/icons/`), not hand-edited. |
...
- **`env(safe-area-inset-*)` matters on installed iOS home-screen apps.** `viewport-fit=cover` (every page's viewport meta) lets content draw under the notch/status bar and home indicator; every sticky/fixed element pinned to a true screen edge — headers, `#toast`, `#travel-controls`, `#recenter-btn`, `#camp-here-btn`, `#info-card` — adds the relevant `env(safe-area-inset-top/bottom)` into its own padding/position so it doesn't get clipped there. A new fixed-position element needs the same treatment.
