...
```js
{
  ...
  durability,         // JSON string, not parsed — see player-state.md's "Gear durability"
  ...
  recentEvents, warStarts, warCooldowns, tribute,                        // JSON strings — see below
  armies, fieldBattles,                                                  // JSON strings (arrays) — see below
  fieldBattleCooldowns,                                                  // JSON string — see below
  savedAt
}
```
...
- **`durability`** — each equipped core Equipment slot's current wear (see [player-state.md](player-state.md)'s "Gear durability"). Same reasoning as `equipped` itself: without this, switching characters on the same device would bleed one character's worn-down gear (or freshly-repaired gear) into another's.
