...
- **`party`** — recruited companion ids for party.html's roster (see [secondary-pages.md](secondary-pages.md)). Always `[]` today since nothing can recruit a companion yet, but included now so a future recruit action doesn't also need a snapshot-shape migration to survive a save.
