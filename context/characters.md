...
- **Play**: `localStorage.setItem('goblinwar_activeCharacterId', row.id)`, writes the row's `data` onto the live `goblinwar_*` keys (`applyCharacterData()` — same field set and same defensive `if(defined)` guards the old `applyLoadedState()` used), then redirects to wherever `?redirect=` says.
...
```js
{
  ...
  territoryControl, warState, lastWarTick, population,  // JSON strings/number — see below
  savedAt
}
```

This is the old local-save-slot shape (see player-state.md) plus two additions:
