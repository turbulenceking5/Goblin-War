...
**The header's Food number now depletes live during the animation, not just at arrival.** `Food` is still only ever actually deducted from `playerInventory` at arrival or at a mid-journey interruption (unchanged from above — nothing about when the real cost lands has changed) — but leaving `#stat-food` frozen at its pre-travel value for the whole animation, then jumping straight to the post-cost number, made a trip look free until it suddenly wasn't. `beginTravel` now captures `startFood = getFoodQty()` before departing, and the same `onProgress` callback that updates `activeTravel.lastSnap` each frame also calls `updateLiveFoodDisplay(startFood, t.foodCost, frac)`, which repaints `#stat-food` with `startFood - t.foodCost*frac` — a smooth linear interpolation that lands exactly on the real post-arrival value at `frac=1`, so there's no discontinuity with the `consumeItem` call that follows. This is purely a display — it never touches `playerInventory` itself, so it can't cause a double deduction. The one place this needed a matching fix: `stopTravel()` spends no Food at all ("No time cost"), so it now calls `refreshPlayerStatUI()` on cancel to restore the true, untouched number — otherwise the live display would be left showing a cost that was never actually charged.
...
- **It still costs Food, at the same effective rate travel does** — `waitTick()` (fired every `WAIT_TICK_MS` = 1 real second by `startWaiting()`'s `setInterval`) accrues `WAIT_HOURS_PER_TICK` (2) in-game hours per tick (×3 while the speed toggle is on), and each tick recomputes `hoursToFoodCost(waitState.totalHours)` against the *running total* of hours waited so far, deducting only the delta since the last tick (`costSoFar - waitState.foodDeducted`) — the same incremental-delta pattern the travel food display uses, just applied to real deductions here instead of a display interpolation, since there's no fixed trip length to interpolate toward. Running out of Food mid-wait auto-stops it with a toast, the same gate travel enforces up front.
...
- **No Health/Stamina effect either way** — waiting doesn't drain them (it's not travel) and doesn't regenerate them (that stays the Inn's own role, so waiting can't be used as a free-heal loop).
